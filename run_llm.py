...
'''
    ...
    # Remove reasoning <think>...</think> if present
...
def run_cot_experiment(domain, prob_size, prob_idx, model, max_tokens, algorithm, temperature):
    ...
    llm_plan_f = os.path.join(BASE_DIR, f"experiments/{domain}/plan/{algorithm}/{domain}{prob_size}_{prob_idx}.pddl")
    ...
    os.makedirs(os.path.dirname(llm_plan_f), exist_ok=True)
...
def run(model, max_tokens, domain, algorithm, temperature, p_num, p_size, p_idx):
    ...
    results_dir = os.path.join(BASE_DIR, f"experiments/{domain}/plan/{algorithm}/")
...
def main():
    ...
    parser.add_argument("--model", type=str, default=default_model, help="Name of LLM model")
