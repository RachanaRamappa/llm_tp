...
Generate {plan_number} different candidate plans for this problem.
...
Each plan should contain only grounded PDDL actions.
...
def apply_action(state, action_str, domprob):
    ...
    if action_name not in domprob.domain.operators:
        raise ValueError(
            f"Invalid PDDL action generated by LLM: {action_name}\n"
            f"Full line: {action_str}"
        )
...
def create_state_tree(domprob, model, plan_number, domain, problem, max_tokens, temperature):
    ...
    for action, prob in actions_with_prob:
        action_clean = action.strip().strip('`').strip()
        if not action_clean or action_clean.startswith("; cost") or action_clean.startswith("```") or "<think>" in action_clean or "</think>" in action_clean:
            continue
        if action_clean.lower().startswith("plan pddl") or action_clean.lower().startswith("plan "):
            if current_plan:
                plans.append(current_plan)
                current_plan = []
        elif action_clean.startswith("(") or ("(" in action_clean and ")" in action_clean):
            current_plan.append((action_clean, prob))
    if current_plan:
        plans.append(current_plan)
